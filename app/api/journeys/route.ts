...
// thinking was still exceeding 60s on its own, so this is raised further
// for headroom. If the account's plan caps functions below this, the
// deploy itself will surface that rather than failing silently at runtime.
...
export async function POST(request: Request) {
  ...
  const internalText = [
    ...
    "", "PACKAGE REQUESTED · الباقة المطلوبة", textLine({ en: "Transport", ar: "النقل" }, submission.transport), textLine({ en: "Accommodation", ar: "الإقامة" }, submission.stays), textLine({ en: "Accommodation rating", ar: "تصنيف الإقامة" }, submission.stayRating), textLine({ en: "Plan should include", ar: "يجب أن تتضمن الخطة" }, submission.planIncludes), textLine({ en: "Package notes", ar: "ملاحظات الباقة" }, submission.packageNotes),
