...
export function FeaturedJourneys({ locale = "en" }: { locale?: "en" | "ar" }) {
  ...
  return <section className="featuredJourneys" id="destinations">
    ...
    <div className="featuredJourneyList">{featured.map((slug, index) => { const item = data.find((destination) => destination.slug === slug)!; const content = highlights[locale][slug as keyof typeof highlights.en]; return <article className="featuredJourney" key={slug}><div className="featuredJourneyImage"><Image src={item.image} alt={item.name} fill sizes="(max-width: 800px) 100vw, 58vw" priority={index < 2} /><span className="featuredNumber">0{index + 1}</span></div><div className="featuredJourneyCopy"><span className="featuredLocation"><MapPin aria-hidden="true" />{content[0]}</span><h3>{item.name}</h3><strong>{content[1]}</strong><p>{content[2]}</p><div><Link className="button gold" href={`${prefix}/design-your-journey?destination=${slug}`}>{ar ? "ابدأ هذه الرحلة" : "Start this journey"} <ArrowRight className="directionArrow" size={16} /></Link><Link className="textLink" href={`${prefix}/destinations/${slug}`}>{ar ? "اكتشف التفاصيل" : "Discover more"} <ArrowRight className="directionArrow" size={15} /></Link></div></div></article>; })}</div>
